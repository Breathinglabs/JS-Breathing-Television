	...
	var ytEmbed = {
		...
		listVideos : function(json,cfg){
			...
			if(json.error){
				...
				for (var i = 0; i < json.data.items.length; i++) {
					...
					var t = document.createTextNode(entry.title);
					a.className = entry.id;		
					...
					if(this.cfg	.player == 'embed' && (!entry.accessControl || entry.accessControl.embed == "allowed")){
						...
						//a.setAttribute("onClick", "testAlert()");
						
						if(a.addEventListener){  
							a.addEventListener('click', function(){var n = $(this).attr('class');playYouVideo(n);} ,false);
						}else if(a.attachEvent){  
							a.attachEvent('click', function(){var n = $(this).attr('class'); playYouVideo(n)} ,false);
						}
					}else{
				...
				if(this.cfg.paging == true){
					...
					if(json.data.totalItems > (json.data.startIndex + json.data.itemsPerPage)){
						...
						if(a.addEventListener){  
							...
							a.attachEvent('', ytEmbed.loadNext.bind(this, {cfg: cfg}));  
						}						
					...
					if(json.data.startIndex > 1){
						...
						if(a.addEventListener){  
							...
							a.attachEvent('', ytEmbed.loadPrevious.bind(this, {cfg: cfg}));  
						}						
				...
				this.message('No YouTube videos found');
			}
		},
		/**
		...
		*/		
		/*createPlayer : function(cfg){
			...
			iframe.setAttribute('src', 'http://www.youtube.com/embed/'+ytPlayerParams.videoId+'?enablejsapi=1&fs=0');//controlbar set
			
			hold.appendChild(iframe);
			//div.insertBefore(hold,div.firstChild);
			$('#playerY').append(hold);
			$('#playerY').css('display','block');
