 $(document).ready(function(){
	$('#search-content').hide();
	$('#breathingApp').css({'width':'900px', 'height':'600px'});
	
	 var currentTitle = window.location.href;
	var fifthParameter = currentTitle.split("#")[6];
	if(fifthParameter.indexOf('%20') > -1)
		{
		  fifthParameter = fifthParameter.replace(/%20/g, " ");
		}
	$('#sub-title').text(fifthParameter);
...
// Load the IFrame Player API code asynchronously.
  ...
  var player;
			...
			var currentUrlAdres = window.location.href;
			var newParameter = currentUrlAdres.split("/")[4];
			var idParameter = newParameter.split("#")[1];
			var firstParameter = newParameter.split("#")[2];
			var secParameter = newParameter.split("#")[3];
			var thirdParameter = newParameter.split("#")[4];
			var fourthParameter = newParameter.split("#")[5];
			
			
			

		
	$("#intro").bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function(){
		breathingAlgorithm.run();
	});
	
	function playBreathingApps(){
	...
	run : function(pfu){
		...
		function goStream(stream){
			...
			javascriptNode.onaudioprocess = function () {
				...
				//Check if function for when exhalation is not detected (off event) is running				
					...
					if(n > fourthParameter){
						player.seekTo(thirdParameter);
						player.playVideo();
					}
		...
		var doEvent = function(){
				return{
					stop: function(){
						player.seekTo(firstParameter);
						$("#loopExpl h1").text("Inhalation loop: blow to play exhalation loop");
					},
					
					start: function(){
						player.seekTo(thirdParameter);
						$("#loopExpl h1").text("Exhalation loop");
